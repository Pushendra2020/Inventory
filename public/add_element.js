        ...
        .then(response => {
            ...
            if (response.ok) {
                response.json().then(data => {
                    if (data.success) {
                        messageDiv.innerHTML = `<p class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400">Component added is successfully</p>`;
                ...
                messageDiv.innerHTML = `<p class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400">${response.statusText}</p>`;
                popup(messageDiv, 2000);
            }
        }).catch(error => {
